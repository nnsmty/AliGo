...
def showimg(ID):
    ...
    showimg = re.findall(r'"imagePathList":\["(.*?)",', html)[0]
    return showimg
...
def get_data(ID):
    ...
    sDic = {"product Name": title,
            "product Price(US $)": Price,
            "formatedPrice(US $)": formatedPrice,
...
def product_data(url, sDic, Name_list):
    name_list = ["product Name", "product Price(US $)", "formatedPrice(US $)", "overview-rating", "reviews", "orders",
                 "Stock"]
    for i in Name_list:
        name_list.append(i)
    if len(Name_list) == 0:
